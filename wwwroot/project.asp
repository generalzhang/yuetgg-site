...
<div id="center">
   ...
   %>
     ...
     <div id="right">
         ...
         <div id="right-bottom-project">
         <ul>
          <%
		   set rs=server.CreateObject("adodb.recordset")
		   page=1
		 if request("pro_num")="" then
		   sql="select * from product"
		   rs.open sql,conn,1,1
		    if not rs.eof then
			  if rs.recordcount mod 6=0 then
			page_count=cint(rs.recordcount/6) 
			 else
			  page_count=cint(rs.recordcount/6)+1
			 end if
			end if
			rs.close
			sql="select top 6 * from product"
		else
		  page=request("pro_num")
		  ...
		  elseif page>=page_count then
		    page=page_count
			n=(page_count-1)*6
			sql="select top  6 * from product where id not in (select top "&n&" id from product)"
		  else
		    n=(page-1)*6
			sql="select top 6 * from product where id not in (select top "&n&" id from product)"
		  end if
		  ...
		  %>
            <li>
            <a href="project_content.asp?id=<%=id%>"><img  src="admin/<%=src%>"  width="239" height="239"/></a>
             <p style="text-align:center;color:#FF0000"><a style="color:#FF0000"  href="project_content.asp?id=<%=id%>"><%=title%></a></p> 
            </li>
            <%
			rs.movenext
			 if rs.eof then
		  ...
		  %>
            <li>
            <a href="project_content.asp?id=<%=id%>"><img  src="admin/<%=src%>"  width="239" height="239"/></a>
             <p style="text-align:center;"><a  style="color:#FF0000" href="project_content.asp?id=<%=id%>"><%=title%></a></p>
            </li>
            <%
			rs.movenext
			next
			...
			%>
             ...
             <div style="color:#000; text-align:center;"><br/><br/>
            <p>
         <a href="project.asp?pro_num=<%=page-1%>&page_count=<%=page_count%>">[上一页]</a>
         <%
		  for i=1 to page_count 
		  response.Write "<a href='project.asp?pro_num="&i&"&page_count="&page_count&"'> ["&i&"] </a> "
		  next
		  rs.close
		  
		 %>
           <a href="project.asp?pro_num=<%=page+1%>&page_count=<%=page_count%>">[下一页]</a>
         </p>
         </div>
         </div>
     </div>     
