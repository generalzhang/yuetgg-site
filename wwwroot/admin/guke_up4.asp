...
sub upload_0()    '使用化境无组件上传类
	...
	for each formName in upload.file
		...
		arrUpFileType=split("gif|jpg|png|bnp|mpg|mpeg|avi|rm|rmvb|mov|wmv|asf|dat|mpa|doc|exl|ppt|zip|rar|txt|mp4","|")
		...
		if EnableUpload=false then
			msg="这种文件类型不允许上传！\n\n只允许上传这几种文件类型：gif|jpg|png|bnp|mpg|mpeg|avi|rm|rmvb|mov|wmv|asf|dat|mpa|doc|exl|ppt|zip|rar|txt|mp4"
			FoundErr=true
		end if
...
Response.Write "<script>alert('上传成功！文件名： "&filename&" ');this.location.href='join-us-add2.asp?filename="&filename&"';</SCRIPT>"
response.end
		set file=nothing
	next
	set upload=nothing
