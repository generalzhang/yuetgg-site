...
%>
  ...
  response.Write("暂时没有相关信息!")
   ...
   %>
        ...
        <center>
        <strong style="font-size:18px;"><%=rs("fname")%>&nbsp;&nbsp;&nbsp;&nbsp;<a href="join-us-delete.asp?id=<%=rs("id")%>">删除</a>&nbsp;&nbsp;&nbsp;&nbsp;
        <a href="join-us-edite.asp?join_id=<%=rs("id")%>">修改</a></strong>
        <br /><p style=" font-size:12px; color:#552c55;">发布人：<%=rs("author")%>&nbsp;|&nbsp;发布时间:<%=rs("time")%></p></center>
		<%=rs("content")%><br /><br />
        附件：<a href="<%=rs("main")%>"><%=rs("main")%></a><br />
  </div><br /><br /><br />
